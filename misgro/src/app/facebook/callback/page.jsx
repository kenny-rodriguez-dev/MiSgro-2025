...
export default function FacebookCallbackPage() {
  ...
  const [message, setMessage] = useState("Procesando Facebook OAuth...");
  ...
  useEffect(() => {
    ...
    if (error) {
      const errorDescription = searchParams.get("error_description") || "Error durante la autenticación con Facebook.";
    ...
    if (!code) {
      setMessage("No se recibió código de Facebook.");
      if (window.opener) {
        window.opener.postMessage({ type: "social-login-error", message: "No se recibió código de Facebook." }, MAIN_FRONTEND_ORIGIN);
        window.close();
      }
      return;
    }
    ...
    const doExchange = async () => {
      try {
        const res = await axios.get(
          `${API_URL}/auth/facebook/callback?code=${code}`
        );
        if (res.data && res.data.success) {
          setMessage("Login con Facebook exitoso. Comunicando a la ventana principal...");
          if (window.opener) {
          ...
          const errorDetail = "Error en login con Facebook: " + (res.data?.message || "Respuesta no exitosa del backend.");
        ...
        const errorDetail = "Error al comunicarse con el backend para el token de Facebook.";
        setMessage(errorDetail);
        if (window.opener) {
          window.opener.postMessage({ type: "social-login-error", message: errorDetail }, MAIN_FRONTEND_ORIGIN);
          window.close();
